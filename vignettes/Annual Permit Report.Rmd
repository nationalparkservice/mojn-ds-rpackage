...
visits.park.data <- visits.park %>%
  ...
  unique() %>%
  dplyr::arrange(SampleFrame, SiteName) %>%
  dplyr::filter(dplyr::case_when(params$wateryear %in% c("2016", "2019", "2022", "2025", "2028") & params$park %in% c("LAKE", "MOJA") ~ SampleFrame %in% c("Annual", "3Yr"),
...
sites.coords <- dplyr::left_join(sites.status, coords, by = "SiteCode") %>%
